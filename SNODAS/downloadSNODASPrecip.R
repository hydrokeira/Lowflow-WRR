...
#Sagehen_shape<-readOGR("/Users/keirajohnson/Box Sync/Keira_Johnson/Sagehenshapefile/layers/globalwatershed.shp")
Coal_shape<-readOGR("/Users/keirajohnson/Box Sync/Keira_Johnson/CoalCshapefile/layers/globalwatershed.shp")

watershed_shape<-Coal_shape

#open list
#precip_HJA_matrix_mm<-list()
#precip_Sage_matrix_mm<-list()
precip_Coal_matrix_mm<-list()
dateList<-list()
...
#datesWanted<-as.Date("2015-03-01")

#use this function to extract data from dat.gz file and output cropped matrix
extract_downloaded_frame<-function(file_name){
  ...
  precip_mm<-crop(precipRaster, watershed_shape)
  precip_mm_mask<-mask(precip_mm, watershed_shape)
  
  return(as.matrix(precip_mm_mask))
  
}

for (i in 3266:length(datesWanted)) {
  ...
  tryCatch({
    ...
    precip_Coal_matrix_mm[[i]]<-extract_downloaded_frame(file_name = file_name)
...
save(precip_Coal_matrix_mm, file = "precip_Coal_matrix_mm.RData")
