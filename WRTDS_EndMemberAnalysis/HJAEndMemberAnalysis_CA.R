...
setwd("/Users/keirajohnson/Box Sync/Keira_Johnson/HJ_Andrews")
...
#write.csv(output1, "HJABaseflow.csv")

wateryearAGG<-aggregate(Q~CDate, output1, FUN = mean)

pdf("EndMemberAnalysisFlow_max_99AnnualBF_CA.pdf")
...
ggplot()+geom_line(output1, mapping=aes(CDate, QBFROminBFmax, color=waterYear))+
  ...
  scale_x_date(date_labels = "%b %d")+ggtitle("QBFROminBFmax")+ylim(0, 20)

ggplot()+geom_line(output1, mapping=aes(CDate, QBFROminBF99, color=waterYear))+
  geom_line(wateryearAGG, mapping = aes(CDate, Q))+
  scale_x_date(date_labels = "%b %d")+ggtitle("QBFROminBF99")+ylim(0, 20)

ggplot()+geom_line(output1, mapping=aes(CDate, QBFRO01BFmax, color=waterYear))+
  geom_line(wateryearAGG, mapping = aes(CDate, Q))+
  scale_x_date(date_labels = "%b %d")+ggtitle("QBFRO01BFmax")+ylim(0, 20)

ggplot()+geom_line(output1, mapping=aes(CDate, QBFRO01BF99, color=waterYear))+
  geom_line(wateryearAGG, mapping = aes(CDate, Q))+
  scale_x_date(date_labels = "%b %d")+ggtitle("QBFRO01BF99")+ylim(0, 20)
...
output9<-output9[,c(1:5,9)]

output9$propGW<-output9$QBFRO01BF99/output9$Q
...
pdf("HJA_C_Q_CA_EndMemberAnalysis.pdf")
...
ggplot()+geom_point(total, mapping = aes(Q, ConcAve), color="blue")+
  geom_point(ROdata, mapping = aes(Q, ConcAve), color="red")+ggtitle("5th Percentile Discrete Specific Calcium (RO)")
