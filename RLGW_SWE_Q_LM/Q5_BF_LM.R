...
#select outcome variables for models predicting SSGW and Q5 using climate parameters
outcome_vars<-c("BFPer", "Q5")

#open lists for coefficient and r2 values to be appended into
coef_df_list_master<-list()
coef_df_list<-list()
r2_df_list_master<-list()
r2_df_list<-list()

#start list - cylce through each site
for (i in 1:length(df_list)) {
  
  #cycle through outcome variables
  for (k in 1:length(outcome_vars)) {
    ...
    #day variables - will pull out for all, colinear with peak SWE
    Day_Vars<-grep("Day", colnames(site_data))
    
    #lag variables - will pull out for Coal Creek, too much structure in data
    Lag_Vars<-grep("lag", colnames(site_data))
    
    #remove snow fraction at Coal Creek - very colinear with all other variables
    SnowFraction<-grep("Snow", colnames(site_data))
    
    #remove BF_lag
    BF_lag<-grep("BFlag", colnames(site_data))
    
    Liquid<-grep("Liquid", colnames(site_data))
    
    #remove variables from Coal Creek (include lag and snow fraction) and 
    #Lookout and Sagehen
    if(i==1){
      
      site_data<-site_data[,-c(1,Day_Vars,Lag_Vars,SnowFraction, Liquid)]
      
    }else{
      
      site_data<-site_data[-1,-c(1, Day_Vars, BF_lag, Liquid)]
      
    }
    
    #pull out precipitation factors when not predicting Q5 at Coal Creek,
    #all rain falls AFTER shoulder season so should not be included in this model
    Precip<-grep("Precip", colnames(site_data))
    
    if(i==1 & outcome_vars[k]=="BFPer"){
      
      site_data<-site_data[,-c(Precip)]
    ...
    #remove the outcome column (Q5 or BFPer) that is not the outcome variable
    remove_this<-setdiff(outcome_vars, outcome_vars[k])
    
    site_data<-site_data[,!colnames(site_data) %in% remove_this]
    
    #create model forumla for AIC
    lm_mod_vars<-formula(paste(outcome_vars[k], "~."))
    
    #put into model
    lm_mod<-lm(lm_mod_vars, site_data)
    ...
    if(i==1 & outcome_vars[k]=="Q5") {
      
      new_lm_input <-"PeakSWE+TotPrecip"
      
    } else{
      
      new_lm_input<-new_lm_input
    ...
    colnames(df_master)<-c(paste0(sites[i], "_", outcome_vars[k], "_pvalue"), 
                           paste0(sites[i], "_", outcome_vars[k], "_VIF"),
                                  paste0(sites[i], "_", outcome_vars[k], "_beta"))
    ...
    colnames(r2_tot)<-paste0(sites[i], "_", outcome_vars[k])
  ...
  coef_df_list_master[[i]]<-coef_df_list
  
  r2_df_list_master[[i]]<-r2_df_list
